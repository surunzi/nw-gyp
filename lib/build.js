...
function build (gyp, argv, callback) {
  ...
  function loadConfigGypi () {
    fs.readFile(configPath, 'utf8', function (err, data) {
      ...
      var versionStr = config.variables.target
  ...
  function copyNodeLib () {
    ...
    var buildDir = path.resolve(nodeDir, buildType)
      ...
      , archNWLibPath = path.resolve(nodeDir, arch, 'nw.lib')
      , buildNWLibPath = path.resolve(buildDir, 'nw.lib')

    mkdirp(buildDir, function (err, isNew) {
      if (err) return callback(err)
      log.verbose('"' + buildType + '" dir needed to be created?', isNew)
      var rs = fs.createReadStream(archNWLibPath)
        , ws = fs.createWriteStream(buildNWLibPath)
      log.verbose('copying "nw.lib" for ' + arch, buildNWLibPath)
      rs.pipe(ws)
