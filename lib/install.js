...
function install (fs, gyp, argv, callback) {
  var release = processRelease(argv, gyp)
  ...
  function go () {
    ...
    mkdir(devDir, function (err, created) {
      ...
      function downloadNwLib (done) {
        log.verbose('on Windows; need to download `' + release.name + '.lib`...')
        var archs = ['ia32']
        if (!isLegacy) archs.push('x64')
      ...
      function downloadNodeLib (done) {
        ...
        archs.forEach(function (arch) {
          var dir = path.resolve(devDir, arch)
          var targetLibPath = path.resolve(dir, 'node.lib')
          var libUrl = release[arch].nodeLibUrl
          var libPath = release[arch].nodeLibPath
          var name = arch + ' node.lib'
          log.verbose('node', 'dir', dir)
          log.verbose('node', 'url', libUrl)
